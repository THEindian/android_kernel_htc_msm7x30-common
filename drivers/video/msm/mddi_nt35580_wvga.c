...
static int nt35580_set_dim = 1;

static struct msm_panel_common_pdata *mddi_nt35580_pdata;
...
static void mddi_nt35580_panel_set_backlight(struct msm_fb_data_type *mfd)
{
...
static int __devinit nt35580_probe(struct platform_device *pdev)
{
	if (pdev->id == 1) {
		mddi_nt35580_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static struct msm_fb_panel_data nt35580_panel_data = {
	.on = mddi_nt35580_panel_on,
	.off = mddi_nt35580_panel_off,
	.set_backlight = mddi_nt35580_panel_set_backlight,
};
...
static struct platform_device this_device = {
	...
	.id = 1,
	.dev = {
		.platform_data = &nt35580_panel_data,
	}
};

static int __init mddi_nt35580_wvga_init(void)
{
	int ret;
	struct msm_panel_info *pinfo;

#ifdef CONFIG_FB_MSM_MDDI_AUTO_DETECT
	if (msm_fb_detect_client("mddi_nt35580_wvga"))
		return -ENODEV;

	if ((0xb9f6 << 16 | 0x5580) != mddi_get_client_id())
		return -ENODEV;
#endif
	...
	pinfo = &nt35580_panel_data.panel_info;
	pinfo->xres = 480;
	pinfo->yres = 800;
...
module_init(mddi_nt35580_wvga_init);
